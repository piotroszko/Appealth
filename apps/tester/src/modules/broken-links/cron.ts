...
async function processNextRequest() {
  ...
  try {
    ...
    const options: BrokenLinksOptions = {
      ...
      averageFromForNotAllowed:
        (request.options?.averageFromForNotAllowed as number) ?? 1,
    };
    ...
    console.log(
      `Completed broken links test ${request._id} for ${request.domain}`,
    );
  } catch (error) {
...
async function saveResultBuckets(
  brokenLinksTestRequestId: string,
...
async function recoverStaleRequests() {
  ...
  if (result.modifiedCount > 0) {
    console.log(
      `Recovered ${result.modifiedCount} stale broken links request(s)`,
    );
  }
}
